...
struct VercelPackager: CommandPlugin {
    ...
    private func buildInDocker(
        ...
        for product in products {
            ...
            try self.execute(
                executable: dockerToolPath,
                arguments: ["run", "--platform", "linux/x86_64", "--rm", "-v", "\(packageDirectory.string):/workspace", "-w", "/workspace", baseImage, "bash", "-cl", buildCommand],
...
private struct Configuration: CustomStringConvertible {
    ...
    public init(
        ...
        if let outputPath = outputPathArgument.first {
            ...
            self.outputDirectory = context.pluginWorkDirectory.appending(subpath: "\(VercelPackager.self)")
        }
